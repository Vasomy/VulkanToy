...
namespace acamcad{
namespace polymesh {
	std::vector<Vertex> PolyMesh::GetVertices()
	{
		std::vector<Vertex>ret;
		ret.resize(vertices_.size());
		int length = ret.size();
		for (int i = 0; i < length; ++i)
		{
			auto& position = vertices_[i]->position();
			auto& texcoord = vertices_[i]->getTexture();
			ret[i].position = {-position.P[0],-position.P[1],-position.P[2]};
			ret[i].texcoord = {texcoord.uv[0],texcoord.uv[1]};
		}
		return std::move(ret);

	}
	std::vector<Index> PolyMesh::GetIndices()
	{
		std::vector<Index>ret;
		int indexNums = numPolygons();
		for (int i = 0; i < indexNums;++i)
		{
			auto* face = polyface(i);
			auto vhandles = polygonVertices(face);
			for (int j = 0; j < vhandles.size(); ++j)
			{
				Index index = vhandles[j]->index();
				ret.push_back(index);
			}
		}
		//std::cout << ret.size() << '\n';
